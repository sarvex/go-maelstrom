...
class Net():
    ...
    def on(self, msg_type, handler):
        ...
        if msg_type in self.handlers:
            raise RuntimeError(f'already have a handler for message type {type}')
...
class Log():
    ...
    def last_term(self):
        ...
        return l['term'] if (l := self.last()) else 0
    ...
    def from_index(self, i):
        ...
        if i <= 0:
            raise LookupError(f'illegal index {i}')
        return self.entries[i - 1:]
...
class KVStore():
    ...
    def apply(self, op):
        ...
        if t == 'cas':
            ...
            res = (
                {'type': 'read_ok', 'value': self.state[k]}
                if k in self.state
                else {'type': 'error', 'code': 20, 'text': 'not found'}
            )
        elif t == 'write':
        ...
        if self.state == 'leader' and self.min_replication_interval < elapsed_time:
            ...
            for node in self.other_nodes():
                ...
                if 0 < len(entries) or self.heartbeat_interval < elapsed_time:
                    log(f'replicating {str(ni)}+ to', node)
